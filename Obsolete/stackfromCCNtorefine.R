...
list_of_dfs <- map(csv_files, ~{
  df <- read.csv(.)
  columns_to_convert <- c("Latitude", "Longitude", "Plot", "core_id", "fraction_nitrogen")
...
selected_columns <- c("Database", "study_id", "site_id", "file_id", "core_latitude", "core_longitude", "core_id", "fraction_nitrogen", "fraction_total_nitrogen", 
                      "depth_min","depth_max","dry_bulk_density","core_year","salinity_class",
...
combined_data <- rename(combined_data,
                        ...
                        Longitude = core_longitude, L_depth_m = depth_min, U_depth_m = depth_max)
...
combined_data3 <- combined_data2 %>% 
  dplyr::select(Source, Site_name,  Plot,
                Latitude, Longitude, U_depth_m, L_depth_m, OC_perc, N_perc, BD_reported_g_cm3, DOI)

#Need all these from CCN: (Source, Site_name, Site, Plot, Habitat_type, Year_collected, 
#Latitude, Longitude, U_depth_m, L_depth_m, OC_perc, N_perc, BD_reported_g_cm3, DOI)
...
write.csv(combined_data3, output_file, row.names = FALSE)
