...
input_data_plot01 <- read.csv("data/raw_data/Schile_2016_DRYAD_plot-info.csv", header = T)
input_data_soil01 <- read.csv("data/raw_data/Schile_2016_DRYAD_soil-data.csv")
##### format data  #####
###plot data
...
SOM_perc = as.numeric(as.character(X_loss_on_ignition_LOI_)), #LOI percent
OC_perc = as.numeric(as.character(X_organic_carbon_OC_)), #percent OC  #AL-changed to numeric
IC_perc = as.numeric(as.character(X_inorganic_carbon_IC_)),#AL-changed to numeric
N_perc = as.numeric(as.character(X_nitrogen_N_)) ,#AL-changed to numeric
...
relocate(Source, Site_name, Site, Plot, Habitat_type, Latitude, Longitude,accuracy_flag, accuracy_code, Emirate, Country, Year_collected, .before = U_depth_m) %>%
...
##Langley- change headings
export_data_merged03<-export_data_merged02 %>%
  mutate(study_id="Schile_et_al_2016",site_id=Site,core_id= Plot  )
