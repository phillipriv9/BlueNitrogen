...
input_data01 <- read.csv("data/raw_data/Xia_2022_SI.csv")
...
study_locations <- read.csv("data/raw_data/Xia_2022_SI_locations.csv")
...
study_properties <- read.csv("data/raw_data/Xia_2022_SI_properties.csv")
...
export_data02 <- export_data01 %>% 
  #AL-Want to include other habitat types  #filter(Habitat_type == "Saltmarsh") %>% 
  filter(OC_perc != "nd") %>% 
...
export_data04 <- export_data03 %>% 
  relocate(study_id, Original_source, site_id, core_id, Habitat_type, Latitude, Longitude, 
